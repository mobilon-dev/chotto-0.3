...
3. **Валидатор sidebar** (`validators/sidebar/`)
...
- ✅ Валидация sidebar items (автоматическая)
...
В верхней части интерфейса (ChatInfo actions) расположены три кнопки:
...
| Sidebar | ✅ | - | 🟠 Оранжевый | ☰ |
| Sidebar | ❌ | Есть | 🔴 Красный | ! (пульсирует) |
...
- Обязательные поля: itemId, icon, notificationCount, selected
- Опциональные: name, notificationColor, isFixedBottom, isFixedTop
- Уникальность: itemId
...
- [x] Валидатор sidebar items создан
- [x] Интеграция в MobilonExtendedChatApp
- [x] UI-кнопки добавлены (3 штуки)
