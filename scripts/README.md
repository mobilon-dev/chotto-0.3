...
Скрипт для комплексной валидации системы тем в проекте chotto-0.3. Выполняет **10 типов проверок** для компонентных тем и дополнительную проверку для **глобальных тем** для обеспечения консистентности и типобезопасности.

### Описание

Скрипт проверяет:

**Компоненты (10 проверок):**
...
10. **Отсутствие глобальных переменных в файлах тем** - в `styles/themes/*.scss` не используется `--chotto-theme-*` как значения компонентных переменных
...
- 📋 **Детальную проверку каждого компонента** - результаты 10 типов валидации
...
#### 10. Валидация глобальных тем (src/themes/*/vars.scss)
...
- **10 типов проверок** на каждый компонент + проверка глобальных тем
