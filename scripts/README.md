...
Скрипт для комплексной валидации системы тем в проекте chotto-0.3. Выполняет **7 типов проверок** для компонентных тем и дополнительную проверку для **глобальных тем** для обеспечения консистентности и типобезопасности.

### Описание

Скрипт проверяет:

**Компоненты (7 проверок):**
...
4. **Использование базовых настроек темы** - нет жестко заданных цветов, шрифтов, отступов, теней
5. **Отсутствие data-theme в файлах стилей** - нет селекторов `[data-theme="..."]` в файлах стилей компонентов
6. **Отсутствие CSS классов в файлах тем** - файлы тем содержат только CSS переменные, а не CSS классы
7. **Отсутствие импортов тем в файлах стилей** - нет прямых импортов тем в файлах стилей компонентов
...
- 📋 **Детальную проверку каждого компонента** - результаты 7 типов валидации
...
#### 8. Валидация глобальных тем (src/themes/*/vars.scss)
...
.component {
  ...
  padding: var(--chotto-theme-spacing-md);
  font-family: var(--chotto-theme-font-family-primary);
}
...
- **7 типов проверок** на каждый компонент + проверка глобальных тем
